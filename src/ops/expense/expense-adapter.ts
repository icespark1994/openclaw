...
function validateClassification(data: unknown): ExpenseClassification {
  ...
  if (!VALID_CATEGORIES.has(obj.category as string)) {
    throw new Error(
      `Invalid category: "${String(obj.category)}". Must be one of: ${[...VALID_CATEGORIES].join(", ")}`,
    );
  }
  ...
  if (!VALID_RISK_LEVELS.has(obj.risk_level as string)) {
    throw new Error(
      `Invalid risk_level: "${String(obj.risk_level)}". Must be one of: ${[...VALID_RISK_LEVELS].join(", ")}`,
    );
  }
...
export async function classifyExpense(
  ...
  if (!apiKey) {
    return {
      ok: false,
      error: "No API key available. Set OPENROUTER_API_KEY or pass options.apiKey.",
    };
  }
