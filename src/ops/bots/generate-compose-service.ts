 ...
 */
export function generateComposeService(
  spec: BotSpec,
  ...
  // Volume-mount the Telegram token file for docker-runtime bots that declare one.
  // Host path → /home/node/.openclaw/secrets/<id>.telegram-token (read-only).
  if (spec.telegram?.tokenFile) {
    const hostPath = spec.telegram.tokenFile.replace(/^~(?=$|\/)/, os.homedir());
    const containerPath = `/home/node/.openclaw/secrets/${spec.id}.telegram-token`;
    volumes.push(`${hostPath}:${containerPath}:ro`);
  }
