 ...
 *
 * If plan.composeProjectDir is set, uses the existing compose project:
 *   docker compose --project-directory <dir> stop
 * Otherwise generates a temporary compose file and runs `docker compose -f <file> down`.
 */
export async function stopBot(
  ...
  const exec = options.exec ?? defaultExec;
  if (plan.composeProjectDir) {
    return exec("docker", ["compose", "--project-directory", plan.composeProjectDir, "stop"]);
  }
  const composePath = await writeComposeFile(plan, options.composeDir);
 ...
 * Otherwise stops then starts via temporary compose file.
 */
export async function restartBot(
  ...
  const exec = options.exec ?? defaultExec;
  if (plan.composeProjectDir) {
    return exec("docker", ["compose", "--project-directory", plan.composeProjectDir, "restart"]);
  }
 ...
 * If plan.containerName is set it takes precedence over the generated name.
...
export async function statusBot(
  ...
  const exec = options.exec ?? defaultExec;
  // Use explicit containerName override when present (e.g. adopted bots with
  // non-standard container names like "finance-bot-finance-bot-1").
  const containerName = plan.containerName ?? plan.composeService.container_name;
