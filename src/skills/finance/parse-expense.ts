/**
 * Stage 10-D: Finance expense parser.
...
function extractCategory(text: string): string {
  ...
  if (/\b(travel|flight|hotel|accommodation|uber|taxi|transport|train|airbnb)\b/.test(n))
    return "Travel";
  if (/\b(office|stationery|supply|supplies)\b/.test(n)) return "Office Supplies";
  return "Other";
}
...
function extractPaymentMethod(text: string): string {
  ...
  if (/\b(credit\s+card|credit)\b|信用卡/.test(n)) return "Credit Card";
  if (/\b(debit\s+card|debit)\b|借记卡/.test(n)) return "Debit Card";
  if (/\balipay\b|支付宝/.test(n)) return "Alipay";
  if (/\bwechat\s*pay\b|微信支付|微信/.test(n)) return "WeChat Pay";
  if (/\bpaypal\b/.test(n)) return "PayPal";
  if (/\bcash\b|现金/.test(n)) return "Cash";
  if (/\bbank\s+transfer\b|转账/.test(n)) return "Bank Transfer";
  return "unknown";
}
...
function extractPaidBy(text: string): string {
  ...
  if (/个人|自己/.test(n) || /\b(personal|myself|self)\b/.test(n)) return "Personal";
  if (/公司/.test(n) || /\b(company|corporate|employer)\b/.test(n)) return "Company";
  return "unknown";
}
...
function extractBusinessPurpose(text: string): string {
  ...
  if (best != null) return capitalise(best);
...
function extractReimbursable(text: string): string {
  const n = norm(text);
  if (/\b(reimbursement|reimbursable|company|client|business|work|报销)\b/.test(n)) return "Yes";
  return "Unknown";
}
...
function extractProjectTag(text: string): string {
  ...
  if (/\b(infra|infrastructure|server|cloud|aws|gcp|azure)\b/.test(n)) return "Infrastructure";
  if (/\b(marketing|ads|advertising|campaign)\b/.test(n)) return "Marketing";
  if (/\b(hr|hiring|recruit|onboard)\b/.test(n)) return "HR";
  return "unknown";
}

function extractReceipt(text: string): string {
  const n = norm(text);
  if (/\b(receipt|领取|收据)\b/.test(n)) return "Yes";
  return "Unknown";
}

function extractInvoice(text: string): string {
  const n = norm(text);
  if (/\b(invoice|发票)\b/.test(n)) return "Yes";
  return "Unknown";
}
...
function extractAssetType(text: string): string {
  ...
  if (/\b(laptop|macbook|computer|device|monitor|keyboard|ipad|tablet|hardware)\b/.test(n))
    return "Equipment";
  if (/\b(software|subscription|saas|app|license|claude|cursor|api)\b/.test(n)) return "Software";
  if (/\b(service|consulting|support|maintenance)\b/.test(n)) return "Service";
  return "unknown";
}

function extractDepreciable(assetType: string): string {
  if (assetType === "Equipment") return "true";
